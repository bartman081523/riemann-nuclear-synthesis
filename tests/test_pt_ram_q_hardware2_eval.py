# -*- coding: utf-8 -*-
"""Tests fuer pt_ram_q_hardware2_eval.py — die gefrorene verdict_map, SYNTHETISCH.

KEIN QPU, KEIN Netzwerk: alle Verdict-Zweige der gefrorenen Map werden auf
synthetischen 90-Circuit-Raw-Dokumenten durchgespielt, deren Counts die
gefrorene Estimator-Kette (n_hat = m*c/S -> fold -> ABSOLUT-FFT) GESETZTREU
durchlaufen (Mirror des Phase-9-Musters, tests/test_pt_ram_q_hardware_eval.py):
  ...
  n_hat[a] = N[a%q]/(d/q) — am Minimalregister d = q direkt die q Labels
  (Idle-Labels ideal leer), groesste-Reste-Rounding auf EXAKT S Shots
  (=> t6: sum n_hat = m exakt).  kappa-Synth: p0-Skalierung der
  Loschmidt-Cal-Paare (kappa_hat = k +- O(1/S)).

Getestet: alle 8 Zweige (CONFIRMED/COARSE/REFUTED/AMPL/VOID/DEGENERAT/
INVALID x3-Mechanismen/UNMATCHED), die Praezedenz-Ordnung, die alternativen
Lesarten, die Phase-10-Diagnostik-Beine (Echo-Leiter r1-Konsistenz,
d-Invarianz W_CROSS KONSISTENT/VERLETZT) und das committete Raw mit
GEFRORENEM Verdict-Pin (REFUTED — nach dem ersten Auswertungs-Lauf
gepinnt, Phase-3c-Disziplin).
"""
import json
import math
import os
...
import pt_ram_q_hardware2_eval as ev

W_B = ev.load_w_b()
W_A = s2b.W_A
LAW_KAPPA = 0.95
S = hw.SHOTS
RAW_MD5_PIN = "16ca44bd02f81cf623862b4910ccedf8"  # Job dartdg5vr3kc73ejcrgg
...
def _class_counts(pt, ratio):
    ...
    q*sum N^2 - m^2 = d*m*r*model_share_reg + m^2 (041-Identitaet); am
    Minimalregister d = q ist per_class = 1 — Label j TRAEGT Klasse j,
    die Idle-Labels (q..2^nq-1) bleiben ideal leer.
    """
    ...
    for j in range(q):
        ...
        for i in range(per_class):
            ...
            if c:
                out[format(a, f"0{s2b.NQ_BY_ARM[f'q{q}_d{d}']}b")] = c
    return out
...
def _q_uniform_counts(nq, q):
    """Struktur-freie Kontrolle: uniform ueber die q REALen Labels
    (Idle-Labels leer) -> Share 0 exakt (must-not-fire)."""
    counts = _lr([S / q] * q, S)
    return {format(j, f"0{nq}b"): c for j, c in enumerate(counts) if c}
...
def _synth_raw(point_cfg=None, control_cfg=None, diagnostik_cfg=None, *,
               md5_override=None, transpile_override=None,
               corrupt_counts=None):
    """Vollstaendiges 90-Circuit-Raw-Dokument (Struktur == s2b.build_...)."""
    point_cfg = point_cfg or {}
    control_cfg = control_cfg or {}
    diagnostik_cfg = diagnostik_cfg or {}
    pts = s2b._point_pts()
    ...
    for c in s2b.build_hardware_circuit_set():
        ...
        if kind == "structure":
            ...
            pt = pts[(arm, P)]
            kappa = cfg["kappa"]
            center = s2b.center_v1(kappa, pt)
            ratio = center + cfg.get("res", 0.0)
            counts = _class_counts(pt, ratio)
        elif kind == "loschmidt":
            arm, P = c["arm"], c["P"]
            cfg = point_cfg.get((arm, P), _point_cfg_default(arm, P))
            counts = _p0_counts(s2b.NQ_BY_ARM[arm],
                                cfg["kappa"] * 0.98)
        elif kind == "echo_ladder":
            arm, P, r = c["arm"], c["P"], c["r"]
            cfg = point_cfg.get((arm, P), _point_cfg_default(arm, P))
            # geometrisches Gesetz kappa_r = kappa_block^r (Synth)
            counts = _p0_counts(s2b.NQ_BY_ARM[arm],
                                (cfg["kappa"] ** r) * 0.98)
        elif kind == "diagnostik_structure":
            arm, P = c["arm"], c["P"]
            cfg = diagnostik_cfg.get((arm, P),
                                     {"kappa": LAW_KAPPA, "res": 0.0})
            pt = dg[(arm, P)]
            ratio = s2b.center_v1(cfg["kappa"], pt) + cfg.get("res", 0.0)
            counts = _class_counts(pt, ratio)
        elif kind == "diagnostik_loschmidt":
            arm, P = c["arm"], c["P"]
            cfg = diagnostik_cfg.get((arm, P),
                                     {"kappa": LAW_KAPPA, "res": 0.0})
            counts = _p0_counts(s2b.NQ_BY_ARM[arm],
            ...
            q_ctrl = int(c["control"].split("_")[1][1:])
            counts = _q_uniform_counts(s2b.NQ_BY_ARM[c["arm"]], q_ctrl)
        ...
        if corrupt_counts == name:
            counts = dict(counts)
            counts["0" * s2b.NQ_BY_ARM[c["arm"]]] = \
                counts.get("0" * s2b.NQ_BY_ARM[c["arm"]], 0) - 192
        entries.append({
            "name": name, "kind": kind, "arm": c.get("arm"), "P": c.get("P"),
            "rep": c.get("rep"), "r": c.get("r"), "shots_actual": S,
            "counts": counts,
        })
    doc = {
        ...
        "backend": "ibm_fez",
        "job_meta": {"job_id": "SYNTH_RAMQ2_EVAL", "backend": "ibm_fez",
                     "shots_per_circuit": S},
        "transpile": transpile_override or {
            "optimization_level": 3, "seed_transpiler": 7,
            "isa_2q_total": 493, "isa_2q_max": 84},
    ...
    doc["counts_md5"] = (md5_override if md5_override is not None
                         else qpu2.counts_md5([e["counts"] for e in entries]))
    return doc
...
class TestSynthSelfCheck:
    ...
    def test_synth_ratio_hits_target(self, tmp_path):
        ...
        rc = s2b.ratio_from_counts(counts, pt, 2)
    ...
    def test_class_profile_share_identity(self, tmp_path):
        ...
        rc = s2b.ratio_from_counts(counts, pt, 3)
        assert rc["share"] >= 0.0
        assert abs(rc["ratio"] - 1.1) < 0.01

    def test_idle_labels_empty_in_synth(self, tmp_path):
        # Am Minimalregister tragen die Idle-Labels KEINE Masse im Synth
        # (gesetzestreu: ideal leer); t6 bleibt exakt.
        counts = _class_counts(s2b._point_pts()[("q5_d5", 433)], 1.0)
        assert "101" not in counts and "110" not in counts \
            and "111" not in counts  # Idle-Labels 5..7 (3 Qubits)
...
class TestVerdictBranches:
    def test_confirmed_all_law_following(self, tmp_path):
        ...
        assert doc["verdict_map_text"] == \
            hw2.load_frozen_prereg()["verdict_map"][ev.V_CONFIRMED]
    ...
    def test_refuted_two_below_sharp(self, tmp_path):
        ...
        cfg = {("q3_d3", 149): {"kappa": LAW_KAPPA, "res": -(W_A + 0.01)},
               ("q3_d3", 197): {"kappa": LAW_KAPPA, "res": -(W_A + 0.01)}}
        doc = _eval_synth(tmp_path, point_cfg=cfg)
    ...
    def test_invalid_amplification(self, tmp_path):
        ...
        pts = s2b._point_pts()
        pt = pts[("q3_d3", 149)]
        ceiling = s2b.center_v1(1.0, pt) + W_B
        cfg = {("q3_d3", 149): {"kappa": LAW_KAPPA, "res": 0.3}}
        doc = _eval_synth(tmp_path, point_cfg=cfg)
        rec = doc["punkte"]["q3_d3|149"]
    ...
    def test_void_calibration_two_low_kappa(self, tmp_path):
        cfg = {k: {"kappa": 0.70, "res": 0.0} for k in s2b._point_pts()}
        doc = _eval_synth(tmp_path, point_cfg=cfg)
    ...
    def test_invalid_t6_mass_violation(self, tmp_path):
        ...
        doc = _eval_synth(tmp_path, corrupt_counts="struct_q3_d3_149_r0")
    ...
    def test_invalid_t5_transpile_record(self, tmp_path):
        doc = _eval_synth(
            ...
            transpile_override={"optimization_level": 3,
                                "seed_transpiler": 7,
                                "isa_2q_total": 999, "isa_2q_max": 84})
...
# === Diagnostik-Beine (Phase 10): Echo-Leiter + d-Invarianz ==============

class TestDiagnostikBeineSynth:
    def test_echo_ladder_r1_shared_and_geometric_fit(self, tmp_path):
        # r=1 ist der gepaarte Loschmidt-Circuit: kappa_r(r=1) == kappa_hat
        # (Anker) per Konstruktion; der geometrische Fit ueber die Synth-
        # Leiter kappa^r liefert kappa_block == kappa (Gesetztreue).
        doc = _eval_synth(tmp_path)
        for arm, l in doc["echo_ladder"].items():
            assert l["r1_shared_with_loschmidt"] is True
            assert l["r1_konsistent_mit_kappa_hat_anker"] is True
            assert abs(l["fit_kappa_block"] - LAW_KAPPA) < 0.02
            assert l["fit_max_res_log"] < 0.05
            assert l["verdict_role"] == "NICHT verdict-tragend (Diagnostik)"

    def test_d_invarianz_law_following_is_konsistent(self, tmp_path):
        # gesetzestreu run2: res_v1_run2 = 0 an allen 13 alten P; die
        # committeden Run-1-Residuen sind <= 0.0668 < W_CROSS = 0.09
        # -> HARDWARE-D_INVARIANZ_KONSISTENT (Regel Eichung, NICHT Verdict).
        doc = _eval_synth(tmp_path)
        di = doc["d_invarianz_bein"]
        assert di["status"] == ev.D_INV_OK
        assert di["n_ok"] == di["n_punkte"] == 13
        assert di["nicht_verdict_tragend"] is True
        assert di["w_cross"] == hw2.W_CROSS == 0.09

    def test_d_invarianz_verletzt_on_big_delta(self, tmp_path):
        # run2-Residuum +0.15 am Diagnostik-Punkt 109 (ALTES P): delta
        # gegen das committede Run-1-Residuum (0.0341) > W_CROSS ->
        # VERLETZT; der Verdict bleibt UNBERUEHRT (gesetzestreu ->
        # CONFIRMED).
        cfg = {("q3_d3", 109): {"kappa": LAW_KAPPA, "res": 0.15}}
        doc = _eval_synth(tmp_path, diagnostik_cfg=cfg)
        di = doc["d_invarianz_bein"]
        assert di["status"] == ev.D_INV_BAD
        assert di["punkte"]["q3_d3|109"]["ok"] is False
        assert di["punkte"]["q3_d3|109"]["delta"] > hw2.W_CROSS
        # NICHT verdict-tragend: der Verdict folgt allein den 13 Punkten
        assert doc["verdict"] == ev.V_CONFIRMED

    def test_diagnostik_integritaet_t3d_t6d_green(self, tmp_path):
        doc = _eval_synth(tmp_path)
        integ = doc["d_invarianz_bein"]["integritaet"]
        assert integ["t3d_max_dev"] <= ev.T3_TOL
        assert integ["t6d_max_dev"] <= ev.T6_MASS_TOL

    def test_diagnostik_circuit_failure_does_not_touch_verdict(self, tmp_path):
        # Korrupte Diagnostik-Counts veraendern den Verdict NICHT (das Bein
        # ist nicht verdict-tragend) — nur der Bein-Status reagiert.
        doc = _eval_synth(tmp_path, corrupt_counts="diag_struct_q5_d5_625")
        assert doc["verdict"] == ev.V_CONFIRMED
        di = doc["d_invarianz_bein"]
        assert di["status"] == ev.D_INV_BAD  # Bein-Integritaet reagiert


# === Committetes Raw: struktur-only ======================================

RAW_EXISTS = pytest.mark.skipif(
    not os.path.exists(qpu2.RAW_PATH), reason="Fez-Raw noch nicht committet")
...
class TestCommittedRaw:
    ...
    def test_eval_contract(self):
        ...
        assert doc["counts_md5_verified"] is True
        assert doc["raw_counts_md5"] == RAW_MD5_PIN
    ...
    @RAW_EXISTS
    def test_eval_phase10_diagnostik_beine_present(self):
        doc = ev.evaluate()
        for arm, l in doc["echo_ladder"].items():
            assert l["r1_shared_with_loschmidt"] is True
            assert l["r1_konsistent_mit_kappa_hat_anker"] is True
            assert l["fit_kappa_block"] is not None
        di = doc["d_invarianz_bein"]
        assert di["nicht_verdict_tragend"] is True
        assert di["w_cross"] == 0.09
        assert di["status"] in (ev.D_INV_OK, ev.D_INV_BAD)
        assert di["integritaet"]["t3d_max_dev"] <= ev.T3_TOL
        assert di["integritaet"]["t6d_max_dev"] <= ev.T6_MASS_TOL


class TestVerdictPin:
    """GEFRORENER Verdict-Pin — NACH dem ersten Auswertungs-Lauf gepinnt
    (Phase-3c-Disziplin: erst sehen, dann pinnen).  Raw-md5 16ca44bd
    (Job dartdg5vr3kc73ejcrgg), w_B' aus dem committeten Freeze B'."""

    @RAW_EXISTS
    def test_verdict_refuted_all_below_sharp(self):
        doc = ev.evaluate()
        assert doc["verdict"] == ev.V_REFUTED
        vi = doc["verdict_inputs"]
        assert vi["n_points"] == 13 and vi["n_evaluated"] == 13
        assert vi["n_below_sharp"] == 13
        assert vi["n_in_sharp"] == 0
        assert vi["n_kappa_low"] == 0
        assert vi["n_above_ceiling_w_b"] == 0
        # Robust unter der groben Kante: 10/13 unter c - w_A -> >= 2
        assert vi["n_below_coarse_w_a"] == 10
        assert doc["alternative_lesarten"]["refuted_mit_w_a"][
            "verdict_waere"] == ev.V_REFUTED
        assert doc["verdict_map_text"] == \
            hw2.load_frozen_prereg()["verdict_map"][ev.V_REFUTED]
        # Alle Kontrollen gruen: die Falsifikation ist NICHT
        # pipeline-bedingt; das Minimalregister hat die Kalibrierung
        # physikalisch eingeloesst (kein Void, im Gegensatz zu Phase 9).
        assert doc["kontrollen"]["t4_negative_must_not_fire"]["ok"] is True
        assert doc["counts_md5_verified"] is True
        assert doc["alternative_lesarten"]["exklusion"] is None

    @RAW_EXISTS
    def test_kappa_all_above_ceiling_register_ordered(self):
        # Der Design-Erfolg des Minimalregisters: ALLE kappa_hat >= 0.81
        # (Phase 9: 13/13 < 0.81 -> VOID).  Register-Tiefe ordnet weiter:
        # 2-Qubit-Werte strikt ueber den 3-Qubit-Werten.
        doc = ev.evaluate()
        k2 = [r["kappa_hat"] for k, r in doc["punkte"].items()
              if k.startswith("q3")]
        k3 = [r["kappa_hat"] for k, r in doc["punkte"].items()
              if k.startswith("q5")]
        for key, rec in doc["punkte"].items():
            assert rec["kappa_hat"] >= s2b.KAPPA_CEILING, key
            assert rec["kappa_ok"] is True
        assert min(k2) > max(k3)

    @RAW_EXISTS
    def test_ladder_r1_konsistent_and_measured_law(self):
        doc = ev.evaluate()
        for arm, l in doc["echo_ladder"].items():
            assert l["r1_konsistent_mit_kappa_hat_anker"] is True
            assert 0.85 < l["fit_kappa_block"] < 1.0
            assert l["fit_max_res_log"] < 0.05

    @RAW_EXISTS
    def test_d_invarianz_status_pinned(self):
        doc = ev.evaluate()
        di = doc["d_invarianz_bein"]
        assert di["status"] == ev.D_INV_BAD
        assert di["n_ok"] == 7 and di["n_punkte"] == 13
        assert di["max_abs_delta"] > hw2.W_CROSS

    @RAW_EXISTS
    def test_v2_diagnostic_one_sided_suppression_pinned(self):
        # Auch die gesetzliche Aer-kalibrierte Form (per-Punkt-Anker +
        # b_P-Steigung) verfehlt auf Hardware — aber EINSEITIG: ALLE 13
        # res_v2 negativ (Suppression, 0 Amplification); die maximale
        # Abweichung 0.0630 ueberschreitet w_B' 0.0279 — die Referenz ist
        # nicht nur die registrierte v1-Missifikation.
        doc = ev.evaluate()
        rv = [r["diagnostisch_v2"]["res_v2"] for r in doc["punkte"].values()]
        assert len(rv) == 13
        assert all(x < 0 for x in rv)
        assert max(-x for x in rv) > W_B